...
public class UpstageRightAuto extends UpstageAutoBase {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        Pose startingPose = new Pose(Distance2.inTiles(0.5, 2.5), Angle.LEFT.neg());
        ...
        Vision vision = new Vision(hardware, Alliance.BLUE);
        ...
        switch (randomization) {
            ...
            case MIDDLE:
                poser.goTo(
                        ...
                        Distance.inTiles(-1.5).add(Distance.inInches(2)).neg()
                ).run();
                break;
            case RIGHT:
                yOffsetAtBackdrop = Distance.inInches(-6);
                poser.goTo(
                        Distance.inTiles(1),
                        Distance.inTiles(-2).add(Distance.inInches(3)).neg()
                ).run();
                break;
        }
        ...
        poser.goTo(
                Distance.inTiles(2),
                Distance.inTiles(-1.5).add(yOffsetAtBackdrop).neg(),
                Angle.BACKWARD.neg()
        ).run();
